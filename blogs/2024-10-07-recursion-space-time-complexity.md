...
```
At first, it looks similar to the previous example, and my monkey brain tries to short-circuit it and think of it as `~2 * fib(n - 1)`, giving a complexity of `O(n)` - which is wrong.
...
Now, that was fun. Let's try this approach on a couple more algorithms.
...
t(n) = 2 * t(n / 2) + c = 2 * (2 * t(n / 4) + c) + c = 2^2 * t(n / 4) + 2c + c = 2^2 * (2 * t(n / 8) + c) + 3c = 2^3 * t(n / 8) + 2^2c + 2c + c

Do you see the pattern? Actually let's get rid of the constants and simplify it.

t(n) = 2 * t(n / 2) = 2 * (2 * t(n / 4)) = 2^2 * t(n / (2^2)) = 2^2 * (2 * t(n / 8)) = 2^3 * t(n / (2^3))

t(n) = 2^k * t(n / (2^k))
If we set `k = log n`, we can get rid of `t()` on right side and get

t(n) = 2^log n * t(1) = n * c = O(n)
...
Pattern is `t(n / (2^k)) + k * c`.

If we set `k = log n`, we can get rid of `t()` on right side and get

t(n) = t(n / (2^log n)) + log n * c = t(1) + log n * c = c + log n * c = O(log n)
